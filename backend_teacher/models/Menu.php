...
class Menu extends \common\models\MenuBase
{
    ...
    {
        $allMenus =  static::find()->where(['status' => 1, 'module' => 3])->all();
//        $menus = [];
...
//        }
    ...
    {
        return static::find()->where(['id' => $menu_id, 'status' => 1, 'module' => 3])->one();
    }
    /**
     * Get Parent menu
     * @param $parent_id
    ...
    {
        return static::find()->where(['parent_id' => $parent_id, 'status' => 1, 'module' => 3])->one();
    }
    /**
    ...
    {
        $allMenus = static::find()->where(['status' => 1, 'module' => 3, 'parent_id' => $parent_id])->orderBy('sort')->all();
        // $menus = [];
        // foreach ($allMenus as $menu) {
        //     if (self::checkMenuPermission($menu)) {
        //         $menus[] = $menu;
